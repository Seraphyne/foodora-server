            ...
            return restValidationService
              ...
              .then((rest) => {
                ...
                res
                  ...
                  .location(
                    path.posix.join(
                      "https://web-production-ad4e.up.railway.app/",
                      `/restaurant/${rest.id}`
                    )
                  )
...
restRouter
  ...
  .patch(jsonBodyParser, (req, res, next) => {
    ...
    //CONST CALLING PHONE VALIDATION
    const phoneError = restValidationService.phoneValidation(
      trimUpdateRest.phone
    );
    if (phoneError) {
      logs.error(phoneError);
      return res.status(400).json({ error: phoneError });
    ...
    restValidationService
      ...
      .then(() => {
        if (trimUpdateRest.name) {
          logs.info(
            `Restaurant name ${trimUpdateRest.name} was updated successfully.`
          );
          res.status(204).end();
        }
        if (trimUpdateRest.phone) {
          logs.info(
            `Restaurant phone ${trimUpdateRest.phone} was updated successfully.`
          );
          res.status(204).end();
        }
      })
...
restRouter
  ...
  .delete(jsonBodyParser, (req, res, next) => {
    ...
    restValidationService
      ...
      .then(() => {
        logs.info(
          `Dish id ${dish_id} from Restaurant id ${restaurant_id} deleted.`
        );
        res.status(204).end();
      })
      .catch(next);
